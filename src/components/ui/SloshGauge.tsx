...
const SloshGauge: React.FC<SloshGaugeProps> = ({
  ...
  const paint = () => {
    ...
    // Convertido a Horizontal!
    const th = reduce.current ? 0 : clamp(t * v * TILT_GAIN, -TILT_MAX, TILT_MAX);
    const lean = (Math.tan((th * Math.PI) / 180) * Hh) / 2;
    const right = x; // 0 to 100
    
    if (liquid.current) {
      liquid.current.style.clipPath = `polygon(0 0, calc(${right}% + ${lean}px) 0, calc(${right}% - ${lean}px) 100%, 0 100%)`;
    }
    if (marker.current) {
       marker.current.style.transform = `translateX(${((L) * W) / 100}px)`;
    }
  };
  ...
  // Convertido a Horizontal!
  const levelAt = (clientX: number, g: Grip) =>
    clamp(((clientX - g.rect.left) / g.scale / (root.current?.offsetWidth || g.rect.width)) * 100, 0, 100);
  ...
  const down = (e: PointerEvent<HTMLDivElement>) => {
    ...
    // Horizontal
    const markerX = rect.left + ((sim.current.L) / 100) * rect.width;
    const g: Grip = {
      id: e.pointerId,
      rect,
      scale,
      band: Math.abs(e.clientX - markerX) <= BAND * scale,
      grab: null,
      at: sim.current.L,
      sent: NaN
    };
  ...
  return (
      ...
      style={
        {
          ...
          '--sg-ticks': ticks,
          '--sg-font': `${clamp(Math.round(height * 0.4), 10, 16)}px`
        } as CSSProperties
      }
      onPointerDown={down}
      onPointerMove={move}
    ...
    >
      ...
      <div
        ref={liquid}
        className="absolute inset-0 [background:var(--sg-liquid)] [clip-path:polygon(0_0,0_0,0_100%,0_100%)]"
        aria-hidden="true"
      >
        {showValue ? (
          <span
      ...
      {ticks > 0 ? (
        <div
          className="pointer-events-none absolute left-0 bottom-0 w-full h-[22%] bg-repeat-x [background-image:linear-gradient(to_right,color-mix(in_srgb,currentColor_22%,transparent)_0_1px,transparent_1px)] [background-size:calc(100%/var(--sg-ticks))_100%]"
          aria-hidden="true"
        />
      ) : null}
