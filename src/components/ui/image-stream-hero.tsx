...
export function ImageStreamHero({
  ...
  const css = React.useMemo(
    () =>
      ...
      .${card}:hover .ish-hover-target {
        transform: translateZ(150px) translateY(-15%) scale(1.15);
      }
      `,
    [right, left, card, p],
  );
  ...
  return (
    <div
      className={cn("relative overflow-hidden group/stream", className)}
      {...props}
          ...
          {[right, left].map((name) =>
            Array.from({ length: cards }, (_, i) => {
              ...
              return (
                <div
                  key={`${name}-${i}`}
                  className={cn(card, "absolute pointer-events-auto group/card cursor-pointer")}
                  style={{
                    left: "50%",
                    top: `${axis}%`,
                ...
                >
                  {img ? (
                    <div className="ish-hover-target w-full h-full relative rounded-[inherit] overflow-hidden border border-white/10 transition-all duration-200 ease-out group-hover/card:shadow-[0_0_40px_10px_var(--hover-color)] group-hover/card:border-2 group-hover/card:border-[color:var(--hover-color)]">
                      ...
                      {/* Inner glow on hover */}
                      <div 
                        className="absolute inset-0 opacity-0 group-hover/card:opacity-40 transition-opacity duration-200"
                        style={{ background: `linear-gradient(to top, var(--hover-color) 0%, transparent 60%)` }}
                      ></div>
                    </div>
                  ) : null}
                </div>
